...
export default function StaffPage() {
  ...
  const [zoomLevel, setZoomLevel] = useState<number>(() => {
    const saved = localStorage.getItem('staffPageZoom');
  ...
  const handleZoomChange = (event: Event, newValue: number | number[]) => {
    ...
    setZoomLevel(zoom);
    localStorage.setItem('staffPageZoom', zoom.toString());
  };
    ...
    <Box>
      ...
      <Grid container spacing={3}>
        {filteredStaff.map((member) => {
          ...
          return (
            <Grid item xs={gridSize.xs} sm={gridSize.sm} md={gridSize.md} lg={gridSize.lg} xl={gridSize.xl} key={member.id}>
